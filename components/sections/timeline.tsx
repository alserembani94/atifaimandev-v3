          ...
          timeline.map((timelineItem: TimelineType, timelineIndex) => (
            <div key={`timeline-${timelineItem.year}`}>
                <div className="flex flex-col items-center">
                  <h6 className="bg-gradient-to-r from-blue-500 to-indigo-500 px-4 py-2 w-max rounded-md text-white mb-4">{timelineItem.year}</h6>
                  <ul className="flex flex-col gap-12">
                    {
                      timelineItem.activities.map((activity, index) => (
                        <li key={`activity_${timelineItem.year}_${index}`} className="flex flex-col gap-2">
